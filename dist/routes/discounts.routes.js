...
router.get("/:id", async (req, res, next) => {
    try {
        ...
        if (!discount)
            throw new ApiError(404, "RESOURCE_NOT_FOUND", "Discount not found");
        res.json(discount);
    }
    catch (e) {
        next(e);
...
router.patch("/:id", authRequired, adminOnly, async (req, res, next) => {
    try {
        ...
        if (req.body.discount_rate !== undefined)
            data.discount_rate = Number(req.body.discount_rate);
        if (req.body.start !== undefined)
            data.start = new Date(req.body.start);
        if (req.body.end !== undefined)
            data.end = new Date(req.body.end);
        if (req.body.is_valid !== undefined)
            data.is_valid = !!req.body.is_valid;
