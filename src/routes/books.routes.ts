...
router.post("/", authRequired, adminOnly, async (req, res, next) => {
  try {
    ...
    if (e?.code === "P2002") return next(new ApiError(409, "DUPLICATE_RESOURCE", "ISBN already exists"));
    next(e);
  }
});
...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (!Number.isNaN(minPrice as any) || !Number.isNaN(maxPrice as any)) {
      where.price = {};
...
router.patch("/:id", authRequired, adminOnly, async (req, res, next) => {
  try {
    ...
    if (!exists) throw new ApiError(404, "RESOURCE_NOT_FOUND", "Book not found");
