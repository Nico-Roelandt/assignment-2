...
router.get("/books/:id/reviews", async (req, res, next) => {
    try {
        ...
        if (!Number.isInteger(bookId))
            throw new ApiError(400, "BAD_REQUEST", "Invalid book id");
    ...
    try {
        ...
        if (req.body.rating !== undefined) {
            ...
            if (!Number.isInteger(rating) || rating < 1 || rating > 5)
                throw new ApiError(400, "BAD_REQUEST", "Invalid rating");
            data.rating = rating;
        }
        if (req.body.comment !== undefined)
            data.comment = req.body.comment === null ? null : String(req.body.comment);
