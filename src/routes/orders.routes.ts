...
router.get("/", authRequired, adminOnly, async (req, res, next) => {
  try {
    ...
    if (req.query.userId !== undefined) {
      ...
      if (Number.isInteger(userId)) where.user_id = userId;
    }
    if (req.query.bookId !== undefined) {
      const bookId = Number(req.query.bookId);
      if (Number.isInteger(bookId)) where.book_id = bookId;
    }
    if (req.query.status !== undefined) where.status = String(req.query.status);
...
router.patch("/:id/status", authRequired, adminOnly, async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(id) || !status) throw new ApiError(400, "BAD_REQUEST", "Invalid body");

    const exists = await prisma.orders.findUnique({ where: { id } });
    if (!exists) throw new ApiError(404, "RESOURCE_NOT_FOUND", "Order not found");
