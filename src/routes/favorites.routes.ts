...
router.post("/", authRequired, async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(book_id)) throw new ApiError(400, "BAD_REQUEST", "Invalid body");
...
router.delete("/:id", authRequired, async (req, res, next) => {
  try {
    ...
    if (!fav) throw new ApiError(404, "RESOURCE_NOT_FOUND", "Favorite not found");
    if (fav.user_id !== userId) throw new ApiError(403, "FORBIDDEN", "Not allowed");
...
router.delete("/by-book/:bookId", authRequired, async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(bookId)) throw new ApiError(400, "BAD_REQUEST", "Invalid bookId");
